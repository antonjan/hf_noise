...
system("rrdtool  graph $basepath$loop_variable$underscore$file_name$underscore$filetimestamp --title 'HF Noise Monitoring (ZR6AIC) http://zr6aic.blogspot.com' --end now --start $starttime --vertical-label \"Power(db)\" --slope-mode -w 500 -h 135 -a PNG DEF:$loop_variable=/home/hfnoise/hf_noise/hf_noise.rrd:$loop_variable:AVERAGE LINE1:$loop_variable#ff0000:$loop_variable VDEF:Last=$loop_variable,LAST 'GPRINT:Last:Last %2.2lf%sdB' VDEF:Min=$loop_variable,MINIMUM 'GPRINT:Min:Min %2.2lf%sdB' VDEF:Peak=$loop_variable,MAXIMUM 'GPRINT:Peak:Max %2.2lf%sdB' VDEF:Average=$loop_variable,AVERAGE 'GPRINT:Average:Avg %2.2lf%sdB' 'COMMENT:\\l' 'COMMENT:Created $now\\r'\n");
$i ++;
} 
